<template>
  <v-container fluid class="pa-0 pr-0">
    <v-card-title style="color: black"
      >{{ title }}
      <v-chip class="ml-3" :color="getColor" dark>{{ count }} </v-chip>
    </v-card-title>
...
export default {
  ...
  computed: {
    countBalcony: {
      get() {
        return this.areaData.bankin.total.balcony_length;
      },
    },
    getColor() {
      return this.title ? "green" : "white";
    },
  },
  created() {},
  data: () => ({
  ...
  methods: {
    emitChanges(item, index) {
      if (!this.bool) {
        this.areaData.bankin.balcony.computeRow(item, index);
        this.areaData.bankin.roofgarden.computeRow(item, index);
        this.areaData.bankin.parapet.computeRow(item, index);
        this.areaData.bankin.getLength();
      } else {
        this.areaDataDB.bankin.balcony.computeRow(item, index);
        this.areaDataDB.bankin.roofgarden.computeRow(item, index);
        this.areaDataDB.bankin.parapet.computeRow(item, index);
        this.areaDataDB.bankin.getLength();
      }
      ...
      //   if(item.part =='FW') {
      //       fw = item.grid
      //       item.total =  Number(Math.round((parseFloat(fw) ) * (0.910) +'e3')+'e-3')
      //   }
      //   else{
      //     ow = item.grid
      //     item.total =Number(Math.round(parseFloat(ow) * parseFloat(0.910)  +'e3')+'e-3')
      //   }
